...
---- 'start_date' и 'end_date' - это начальная и конечная даты периода в формате YYYY-MM-DD.
--task1
SELECT * FROM ORDERS WHERE ORDER_DATE BETWEEN date('2007-01-01') AND date('2008-01-01');
...
--task4
SELECT * FROM PRODUCTS WHERE PRICE > 4400 ORDER BY PRICE DESC;
...
--task 12
SELECT CUSTOMERS.COMPANY, COUNT(*) AS ORDER_COUNT
FROM ORDERS
...
--task 13
SELECT SALESREPS.NAME, AVG(ORDERS.AMOUNT) AS AVERAGE_ORDER_PRICE
FROM ORDERS
...
FROM PRODUCTS P
WHERE P.PRICE = (SELECT MAX(PRICE) FROM PRODUCTS WHERE MFR_ID = P.MFR_ID);


--task 17
SELECT CUSTOMERS.COMPANY, PRODUCTS.DESCRIPTION, PRODUCTS.MFR_ID, ORDERS.QTY, ORDERS.AMOUNT
FROM ORDERS
JOIN CUSTOMERS ON ORDERS.CUST = CUSTOMERS.CUST_NUM
...
SELECT SALESREPS.*
FROM SALESREPS
WHERE SALESREPS.EMPL_NUM NOT IN (SELECT DISTINCT ORDERS.REP FROM ORDERS);
...
JOIN CUSTOMERS ON ORDERS.CUST = CUSTOMERS.CUST_NUM
WHERE CUSTOMERS.COMPANY = 'JCP Inc.';
...
--task 22
SELECT DISTINCT SR1.NAME, SR2.NAME
FROM SALESREPS SR1, SALESREPS SR2, ORDERS O1, ORDERS O2
WHERE SR1.EMPL_NUM < SR2.EMPL_NUM
AND SR1.EMPL_NUM = O1.REP
AND SR2.EMPL_NUM = O2.REP
AND O1.CUST = O2.CUST;
...
SELECT CUST
FROM ORDERS
GROUP BY ORDER_DATE, CUST
HAVING COUNT(*) > 1;


--task 24
SELECT REP_OFFICE, SUM(AMOUNT) AS TOTAL_SALES
FROM ORDERS
JOIN SALESREPS ON ORDERS.REP = SALESREPS.EMPL_NUM
GROUP BY REP_OFFICE
...
--task 29
SELECT ORDERS.ORDER_NUM, ORDERS.QTY
FROM ORDERS
JOIN SALESREPS ON ORDERS.REP = SALESREPS.EMPL_NUM
JOIN OFFICES ON SALESREPS.REP_OFFICE = OFFICES.OFFICE
WHERE OFFICES.REGION = 'Eastern'
ORDER BY ORDERS.QTY ASC;
...
SELECT *
FROM PRODUCTS
WHERE MFR_ID NOT IN (SELECT MFR_ID
                     FROM PRODUCTS
                     JOIN ORDERS ON PRODUCTS.MFR_ID = ORDERS.MFR
                     JOIN CUSTOMERS ON ORDERS.CUST = CUSTOMERS.CUST_NUM
                     WHERE COMPANY = 'JCP Inc.');
...
FROM SALESREPS SR1
JOIN ORDERS O1 ON SR1.EMPL_NUM = O1.REP
LEFT JOIN ORDERS O2 ON SR1.EMPL_NUM = O2.REP AND strftime('%Y', O2.ORDER_DATE) = '2007'
WHERE strftime('%Y', O1.ORDER_DATE) = '2008' AND O2.ORDER_NUM IS NULL;

--task 33.2
SELECT DISTINCT SR1.EMPL_NUM, SR1.NAME
FROM SALESREPS SR1
JOIN ORDERS O1 ON SR1.EMPL_NUM = O1.REP AND strftime('%Y', O1.ORDER_DATE) = '2008'
WHERE SR1.EMPL_NUM NOT IN (
   SELECT DISTINCT SR2.EMPL_NUM
   FROM SALESREPS SR2
   JOIN ORDERS O3 ON SR2.EMPL_NUM = O3.REP AND strftime('%Y', O3.ORDER_DATE) = '2007'
);

--task 34.1
SELECT DISTINCT C1.COMPANY
FROM CUSTOMERS C1
JOIN ORDERS O1 ON C1.CUST_NUM = O1.CUST AND strftime('%Y', O1.ORDER_DATE) = '2007'
LEFT JOIN ORDERS O2 ON C1.CUST_NUM = O2.CUST AND strftime('%Y', O2.ORDER_DATE) = '2008'
...
FROM CUSTOMERS C1
JOIN ORDERS O1 ON C1.CUST_NUM = O1.CUST AND strftime('%Y', O1.ORDER_DATE) = '2007'
WHERE C1.CUST_NUM NOT IN (
   SELECT DISTINCT C2.CUST_NUM
   FROM CUSTOMERS C2
   JOIN ORDERS O3 ON C2.CUST_NUM = O3.CUST AND strftime('%Y', O3.ORDER_DATE) = '2008'
);
...
FROM CUSTOMERS C1
JOIN ORDERS O1 ON C1.CUST_NUM = O1.CUST AND strftime('%Y', O1.ORDER_DATE) = '2007'
JOIN ORDERS O2 ON C1.CUST_NUM = O2.CUST AND strftime('%Y', O2.ORDER_DATE) = '2008';

--task 35.2
SELECT DISTINCT C1.COMPANY
FROM CUSTOMERS C1
WHERE C1.CUST_NUM IN (
   SELECT DISTINCT O1.CUST
   FROM ORDERS O1
   JOIN ORDERS O2 ON O1.CUST = O2.CUST
   WHERE strftime('%Y', O1.ORDER_DATE) = '2007' AND strftime('%Y', O2.ORDER_DATE) = '2008'
);

/*
 В SQLite для объявления столбцов таблицы используется ключевое слово COLUMN после имени столбца, а не перед ним, как в Oracle.
В SQLite тип данных VARCHAR заменен на TEXT.
В SQLite не поддерживаются типы данных CLOB, BLOB, TIMESTAMP, INTERVAL, RAW и LONG RAW, которые есть в Oracle.
 В SQLite функцию COUNT можно вызывать без указания аргумента, чтобы подсчитать количество строк в таблице. В Oracle же необходимо указать, какой столбец подсчитывать.
В SQLite нет функции NVL, вместо нее используется функция IFNULL.
 В SQLite ключевое слово LIMIT используется для ограничения количества возвращаемых строк, в то время как в Oracle используется ключевое слово ROWNUM.
В SQLite нет оператора MINUS, вместо него используется оператор EXCEPT.
 В SQLite оператор UNION требует, чтобы количества и типы столбцов в объединяемых таблицах совпадали, в то время как в Oracle количества столбцов могут отличаться, но типы должны быть совместимыми.
В SQLite не поддерживается оператор NATURAL JOIN.
 В SQLite функция LIKE не поддерживает ключевое слово ESCAPE.
В SQLite оператор IS NULL используется для проверки на NULL, а не IS [NOT] NULL, как в Oracle.
В SQLite не поддерживается оператор DECODE.
